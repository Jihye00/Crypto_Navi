...
async function getSwappedAmount(txhash, tokenname) {
    ...
    for (let n in tx.logs) {
      if ((tx.logs[n].address).toUpperCase() == (test.TOKEN_ADDRESS[tokenname]).toUpperCase()) {
        ...
        console.log(tokenname + " swapped : " + shifts.lshift(res, type.TOKEN_DECIMAL[tokenname]));

        return await shifts.lshift(res, type.TOKEN_DECIMAL[tokenname]);
      }
    }
...
//             amount_Ksp = await getSwappedAmount(await swap(params[0], params[1], amount_Ksp, params[2]), params[1]);
//             amount_Def = await getSwappedAmount(await swap(params[0], params[1], amount_Def, params[4]), params[1]);

//             amount = safemath.safeAdd(amount_Ksp, amount_Def);
//         }
//         else if (params[3] != 0){
//             console.log("2")
//             amount = await getSwappedAmount(await swap(params[0], params[1], amount, params[2]), params[1]);
//         }
//         else if (params[5] != 0){
//             console.log("3")
//             amount = await getSwappedAmount(await swap(params[0], params[1], amount, params[4]), params[1]);
//         }
...
// SmartSwapRouting('KUSDT', 'KLAY', "3.7ç31073");
// 0.
