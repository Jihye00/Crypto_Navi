...
//             if(t_str[0] == this.currency_list[k]){
    ...
    calc(T, from, a){
        ...
        var from_idx = index_finder(from);
        var new_matrix = [];
        for(var i5=0; i5<this.size; i5++){
            var row = [];
            for(var j5=0; j5<this.size; j5++){
                if(i5==j5) row.push({'money' : 0, 'path' : [], 'slippage': 1});
            ...
            for(var i6=0; i6<this.size; i6++){
                for(var j6=0; j6<this.size; j6++){
                    for(var k6=0; k6<this.size; k6++){
                        if(i6 == j6 || j6 == k6) continue;
                        if(this.matrix[i6][j6]['money'] == 0){
                            var ratio3 = SwapMatrix[i6][j6].ratio(new_matrix[from_idx][i6]['money']);
                        ...
                        if(i6 == k6 && new_matrix[i6][i6]['path'].length > 0) {
    ...
    print(){
        for(var i7=0; i7<this.size; i7++){
            for(var j7=0; j7<this.size; j7++){
                console.log(this.currency_list[i7], '=>', this.currency_list[j7], ':', this.matrix[i7][j7]['path'], this.matrix[i7][j7]['money'], 'slippage:', this.matrix[i7][j7]['slippage']);
            }
            console.log('\n');
        }
    }
